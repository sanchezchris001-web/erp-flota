...
def registrar_movimiento(accion, obs=""):
    conn = get_db()
    cur = conn.cursor()
    cur.execute("INSERT INTO movimientos(accion,usuario,fecha,obs) VALUES(%s,%s,%s,%s)",(
        accion,
...
def cambiar_estado_unidad():
    ...
    cur.execute("UPDATE unidades SET estado=%s WHERE id=%s",(d["estado"],d["unidad_id"]))
...
def exportar_excel():
    ...
    wb=Workbook()
    ws=wb.active
    ws.append(["Acción","Usuario","Fecha","Obs"])
